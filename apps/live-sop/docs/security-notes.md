...
| Arbitrary constructor injection via `deserializeErrors()` in SSR hydration | No — no SSR. `src/main.tsx` uses `createRoot`, never `hydrateRoot` |
...
- `src/components/Layout.tsx` — `navigate(`/${targetPath}`)` where `targetPath`
  derives from `TabId`, a closed union backed by a hardcoded `sections` array.
- `src/App.tsx` — `navigate('/gear')`, a literal.
...
Last assessed against `react-router-dom@7.15.1`.
