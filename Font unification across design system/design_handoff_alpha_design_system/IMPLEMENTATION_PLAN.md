...
1. **`src/components/site-header.tsx`** — ref §06, **rewritten: see `CHANGES-v2.1.md` §1**.
   Fully rounded pill bar (radius 999px, 30px once the bar is under ~1010px wide), a
   sliding white indicator behind the hovered/open nav item, three dropdown panels
   (recipe filters / camera catalogue / features), search that grows 290 → 420px on
   focus with a violet focus ring, account menu at elevation 2 under the avatar, rows
   44px. Drop emoji from the category labels (`📦 📷 🔍 🎙️ 🎧 🔊`) as you touch them.
...
4. **Cameras list + compare** (`src/app/[locale]/cameras/…`) — refs §03, §04,
   **§03 rebuilt: see `CHANGES-v2.1.md` §2**. The facet rail is gone: the default view is
   a 3-column card grid identical in structure to the recipe card, with the old facet
   groups as a horizontal chip rail above it. `specsMissing` renders an em dash. Better
