        ...
        <main className="flex-1 w-full max-w-[110rem] mx-auto px-4 sm:px-6 lg:px-8 py-5 sm:py-6 flex flex-col gap-5">
          ...
          <div className="flex items-center justify-between gap-4 border-b border-white/10 pb-3">
            <div className="flex items-center gap-2 text-xs text-slate-100 font-mono font-bold">
              <Link href={isVi ? '/vi' : '/'} className="hover:text-amber-300 transition-colors">
                ColorLab
              </Link>
              <span>/</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
            ...
            <div className="lg:col-span-7 flex flex-col gap-5">
              ...
              {product.specs && <SpecTable specs={product.specs} t={t} locale={locale} />}
            </div>
...
function SpecTable({
  ...
  specs: ProductSpecs;
  t: Awaited<ReturnType<typeof getTranslations<'cameras'>>>;
  locale: string;
}) {
  const row = specs as unknown as Record<string, string | null>;

  /* An unstated spec renders as "not published" rather than as "N/A" or a
     hidden row. The gap is information: this data comes from pages that vary in
     completeness, and a placeholder that reads like a value hides which of the
     two it is. Same rule the catalogue modal follows. */
