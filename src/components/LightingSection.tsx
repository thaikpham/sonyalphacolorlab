...
const WBStep: React.FC<{ num: string; title: string; desc: string }> = ({ num, title, desc }) => (
  <div className="flex gap-6 group">
    <div className="flex flex-col items-center">
      <div className="w-8 h-8 rounded-full bg-white/[0.03] border border-white/[0.06] flex items-center justify-center text-xs font-bold text-white/40 group-hover:bg-accent-deeper group-hover:text-white group-hover:border-accent-deep transition-all duration-300">
        {num}
      </div>
      <div className="w-px h-full bg-white/[0.06] my-2"></div>
    </div>
    <div className="pb-10">
      <h5 className="text-sm font-bold text-white mb-2 group-hover:text-accent-soft transition-colors">{title}</h5>
    ...
    <div className="space-y-12 pb-20 w-full animate-fade">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 lg:gap-12">
        <div className="xl:col-span-8 space-y-10">
          ...
          <div className="glass-card rounded-[40px] p-8 lg:p-10">
            <div className="flex items-center justify-between mb-8">
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-xl bg-accent-deep/10 flex items-center justify-center">
                  <Icon name="thermostat" className="text-accent-mid text-xl" />
                </div>
            ...
            <div className="flex gap-4 lg:gap-6 mb-10">
              <KelvinIndicator temp={3200} label="Warm Yellow (Studio)" color="bg-warning" />
            ...
            <div className="p-6 bg-white/[0.02] rounded-2xl border border-white/[0.06] flex items-start gap-4">
              <Icon name="info" className="text-accent-mid" />
          ...
          <div className="glass-card rounded-[40px] p-8 lg:p-10 relative overflow-hidden">
            <div className="absolute top-0 right-0 p-12 opacity-5 pointer-events-none">
              <Icon name="palette" className="text-[180px] text-accent-deep" />
            </div>
            ...
            <div className="flex items-center gap-4 mb-10 border-b border-white/[0.06] pb-8">
              <div className="w-14 h-14 rounded-[20px] bg-white/[0.02] flex items-center justify-center border border-white/[0.06]">
                <Icon name="colorize" className="text-accent-mid text-2xl" />
              </div>
              <div>
        ...
        <div className="xl:col-span-4 space-y-6">
          <div className="glass-card rounded-[40px] p-8 flex flex-col items-center text-center">
            ...
            <div className="relative w-48 h-48 mx-auto mb-10">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="w-16 h-16 bg-[#181818] border border-white/10 rounded-full flex items-center justify-center shadow-[0_0_40px_rgba(255,255,255,0.05)]">
                  <Icon name="person" className="text-accent-mid text-2xl" />
                </div>
              </div>
              {/* Light Positions */}
              <div className="absolute -top-4 -right-4 w-12 h-12 bg-warning-soft rounded-2xl flex flex-col items-center justify-center shadow-lg animate-pulse">
                <span className="text-[9px] font-black text-black">KEY</span>
              </div>
              <div className="absolute top-1/2 -left-6 -translate-y-1/2 w-12 h-12 bg-info/30 rounded-2xl flex flex-col items-center justify-center border border-info-soft/20">
                <span className="text-[9px] font-black text-white">FILL</span>
              </div>
              <div className="absolute -bottom-4 right-1/4 w-12 h-12 bg-accent-deeper/30 rounded-2xl flex flex-col items-center justify-center border border-accent-deep/20">
            ...
            <div className="w-full space-y-3 text-left">
              {(['clean', 'warm', 'cinematic'] as const).map((key) => (
                ...
                >
                  <Icon name={moods[key].icon} className={`${activeMood === key ? 'text-black' : 'text-accent-mid'}`} />
          ...
          <div className="p-6 bg-gradient-to-br from-accent-deeper/10 to-transparent rounded-[32px] border border-accent-deep/10">
            <h5 className="text-[10px] font-bold text-accent-mid uppercase mb-3 tracking-widest">Lưu Ý Kỹ Thuật (Pro Tip)</h5>
