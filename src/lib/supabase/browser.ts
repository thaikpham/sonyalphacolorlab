 ...
 *
 * It talks to the *control* plane and nothing else. The content project has no
 * Auth configured, issues no session, and never sees a control-plane JWT — the
 * browser cannot reach it at all, because every content write goes through this
 * application's route handlers, which re-verify the session server-side.
 *
...
export function authBrowser(): SupabaseClient | null {
